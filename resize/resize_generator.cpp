...
class ResizeWM: public Halide::Generator<ResizeWM> {
public:
    ...
    Var oh, ow, c, y, oh_outer, oh_inner, ow_outer, ow_inner;
    Func phase1, input_clamped;
    void generate() {
        Expr sh = input.height(), sw = input.width();
        Expr ch = cast<float>(sh) / output_height;
        Expr cw = cast<float>(sw) / output_width;
        Expr ih = oh * ch, iw = ow * cw;
        Expr ih_s = cast<int>(floor(ih)),
             iw_s = cast<int>(floor(iw));
        Expr ih_f = ih - ih_s, iw_f = iw - iw_s;
        input_clamped = BoundaryConditions::repeat_edge(input);
        phase1(ow, y, c) =
    ...
    void schedule() {
        // output.tile(ow, oh, ow_outer, oh_outer, ow_inner, oh_inner, 8, 8);
        // phase1.compute_root();
        // phase1.compute_at(output, oh);
        /*
        output.split(oh, oh_outer, oh_inner, 4);
        output.vectorize(ow, 4);
        phase1.store_at(output, oh_outer);
        phase1.compute_at(output, oh_inner);
        */
        Target target = get_target();
        printf("has_gpu_feature=%d\n", target.has_gpu_feature());
        if (target.has_gpu_feature()) {
            phase1.compute_inline();
            output.gpu_tile(ow, oh, ow_outer, oh_outer, ow_inner, oh_inner,
                    16, 16);
        } else {
            output.vectorize(ow, 4).unroll(ow, 4);
        }
        /*
        input.dim(0).set_bounds_estimate(0, 1024);
        input.dim(1).set_bounds_estimate(0, 1024);
        input.dim(2).set_bounds_estimate(0, 3);
        output.estimate(oh, 0, 1024);
        output.estimate(ow, 0, 1024);
        */
    }
};
...
HALIDE_REGISTER_GENERATOR(ResizeWM, resizewm);
